...
export default function Sidebar() {
  ...
  return (
    <div className="flex">
      <div className={`bg-white h-screen p-5 pt-1 flex flex-col text-white overflow-hidden ${open ? "w-72" : "w-20"} duration-300 relative border-b border-gray-200`}>
      {/* <FiArrowLeft className={`bg-blue-900 text-white text-xl rounded-full absolute right-0 top-5 border border-blue-900 cursor-pointer
          ${!open && "rotate-180"}`}
        ...
        <div className='inline-flex items-center gap-2 px-1 py-3'>
          <img
                ...
                onClick={() => setOpen(!open)} 
          />
          <h1 className={`text-blue-900 origin-left font-bold text-2xl cursor-pointer duration-300 ${!open && "scale-0"}`} onClick={() => setOpen(!open)} >
            MindTalk
          </h1>
        </div>
